...
@section('content')
    ...
    @else
        <div class="grid grid-cols-4 gap-4 mt-10">
            @foreach ($doghealthcares as $doghealthcare)
                <div class="border border-gray-200">
                    <div>
                        <a href="{{ route('products.detail', $doghealthcare->id) }}">
                            <img src="{{ asset('/images/' . $doghealthcare->images) }}" alt="" class="w-full h-[350px] object-cover">
                        </a>
                    </div>
                    <div class="bg-[#48b194] p-4">
                        <h1 class="font-bold text-[#602b05]">{{ $doghealthcare->name }}</h1>
                        <div class="flex justify-between items-end mt-2">
                            <div>
                                <a href="{{ route('products.detail', $doghealthcare->id) }}">
                                    <h1 class="font-semibold text-gray-100">${{ $doghealthcare->price }}</h1>

...
@section('content')
    ...
    @else
        <div class="grid grid-cols-4 gap-4 mt-10">
            @foreach ($dogtreats as $dogtreat)
                <div class="border border-gray-200">
                    <div>
                        <a href="{{ route('products.detail', $dogtreat->id) }}">
                            <img src="{{ asset('/images/' . $dogtreat->images) }}" alt="" class="w-full h-[350px] object-cover">
                        </a>
                    </div>
                    <div class="bg-[#48b194] p-4">
                        <h1 class="font-bold text-[#602b05]">{{ $dogtreat->name }}</h1>
                        <div class="flex justify-between items-end mt-2">
                            <div>
                                <a href="{{ route('products.detail', $dogtreat->id) }}">
                                    <h1 class="font-semibold text-gray-100">${{ $dogtreat->price }}</h1>
